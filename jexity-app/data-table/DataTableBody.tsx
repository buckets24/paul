...
export interface DataTableBodyProps<D extends Record<string, unknown>> extends DataTableInfiniteLoader {
...
function DataTableBody<D extends Record<string, unknown>>({
  ...
  const renderRow = useCallback(
    ({ index, style }: PropsWithChildren<ListChildComponentProps>) => {
      ...
      prepareRow(row);
      const { key, style: rowStyle, ...rowProps } = row.getRowProps({
        style,
      });
        ...
        >
          ...
          >
            {row.cells.map((cell) => {
              ...
              const rowToggleProps: TableToggleRowsSelectedProps = row.getToggleRowSelectedProps();
                  ...
                  <Flex alignItems="center" justifyContent="center" h="100%">
                    <Checkbox
                      ...
                      onChange={(e: ChangeEvent<Element>) => {
                        // @ts-expect-error: The typings of react-table seems to be inaccurate
                        rowToggleProps.onChange(e);
                      }}
                    />
                  </Flex>
                ) : (
